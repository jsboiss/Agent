...
public sealed class MemoryCandidateReviewer(IMemoryStore memoryStore) : IMemoryCandidateReviewer
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            reviews.Add(await ReviewCandidate(request.ConversationId, candidate, cancellationToken));
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var existingMemories = await memoryStore.Search(
            ...
            cancellationToken);
        var duplicate = existingMemories.FirstOrDefault(x => IsDuplicate(x.Text, candidate.Text));
    ...
    private static bool IsDuplicate(string existingText, string candidateText)
    {
